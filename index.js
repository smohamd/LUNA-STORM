...
app.use(express.static('STORM ROX')); // لخدمة ملفات HTML وCSS
const VERIFY_TOKEN = 'bot';
const PAGE_ACCESS_TOKEN = fs.readFileSync('token.txt', 'utf8').trim();
const COMMANDS_PATH = path.join(__dirname, 'commands');
const adminUIDs = JSON.parse(fs.readFileSync('admins.json', 'utf8')).admins; 
...
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'STORM ROX', 'index.html'));
});
